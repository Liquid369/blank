...
bool CMasternode::IsInputAssociatedWithPubkey() const
{
    ...
    if(GetTransaction(vin.prevout.hash, txVin, hash, true)) {
        for (const CTxOut& out : txVin->vout) {
                if (out.nValue == GetMNCollateral(chainActive.Height()) && out.scriptPubKey == payee) return true;
        }
    }
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if (Params().NetworkIDString() == CBaseChainParams::MAIN) {
        if (addr.GetPort() != 22618) return false;
    } else if (addr.GetPort() == 22618)
        return false;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int nChainHeight, int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 DEV tx got MASTERNODE_MIN_CONFIRMATIONS
