...
#ifndef RBX_STAKEINPUT_H
#define RBX_STAKEINPUT_H
...
public:
    CRbxStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CRbxStake* NewRbxStake(const CTxIn& txin);
    ...
    bool IsZRBX() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //RBX_STAKEINPUT_H
