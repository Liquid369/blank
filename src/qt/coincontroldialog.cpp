...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select DEV Outputs to Spend" :
            "Select Shielded DEV to Spend");
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show Deviant address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
