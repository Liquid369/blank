...
QString formatBalanceWithoutHtml(CAmount amount, int nDisplayUnit, bool iszdev)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, iszdev)) : BitcoinUnits::floorWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, iszdev);
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter Deviant address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no DEV: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::DEV, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert deviant:// to deviant:
    //
    //    Cannot handle this later, because deviant:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::DEV, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    if (gArgs.GetBoolArg("-testnet", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Deviant (testnet).lnk";
    else if (gArgs.GetBoolArg("-regtest", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Deviant (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "DEV.lnk";
}
...
{
    return GetAutostartDir() / "deviant.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a deviant.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=Deviant (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=Deviant (regtest)\n";
        else
            optionFile << "Name=DEV\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the deviant app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add deviant app to startup item list
...
bool isExternal(QString theme)
{
    ...
    return (theme.operator!=("default-dark") && theme.operator!=("default-dark"));
}
