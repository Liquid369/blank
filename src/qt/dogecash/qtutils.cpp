...
void setSortTxTypeFilter(QComboBox* filter, SortEdit* lineEditType)
{
    ...
    filter->addItem(QObject::tr("Dev Fee"), TransactionFilterProxy::TYPE(TransactionRecord::DevReward));
...
void setTheme(bool isLight)
{
    ...
    settings->setValue("theme", isLight ? "default-dark" : "default-dark");
...
QColor getRowColor(bool isLightTheme, bool isHovered, bool isSelected)
{
    if (isLightTheme) {
        if (isSelected) {
            return QColor("#9c7019");
        } else if (isHovered) {
            return QColor("#bababa");
        } else {
