...
#include "qt/deviant/forms/ui_snackbar.h"
#include "qt/deviant/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(DEVGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &DEVGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
