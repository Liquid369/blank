...
BOOST_AUTO_TEST_CASE(TransparentToSapling)
{
    ...
    // 0.5 t-RBX in, 0.4 shielded-RBX out, 0.1 t-RBX fee
...
BOOST_AUTO_TEST_CASE(SaplingToSapling) {
    ...
    // --- 0.4 shielded-RBX in, 0.299 shielded-RBX out, 0.1 shielded-RBX fee, 0.001 shielded-RBX change (added to fee)
    ...
    // --- Now try with 1 shielded-RBX in, 0.5 shielded-RBX out, 0.1 shielded-RBX fee, 0.4 shielded-RBX change
...
BOOST_AUTO_TEST_CASE(FailsWithNegativeChange)
{
    ...
    // Generate a 0.5 RBX note
    auto testNote = GetTestSaplingNote(pa, 59990000);

    // Fail if there is only a Sapling output
    // 0.5 shielded-RBX out, 0.1 t-RBX fee
    auto builder = TransactionBuilder(consensusParams, 1);
    ...
    // 0.5 t-RBX out, 0.1 t-RBX fee, 0.59999 shielded-RBX in
    builder.AddSaplingSpend(expsk, testNote.note, testNote.tree.root(), testNote.tree.witness());
    BOOST_CHECK_EQUAL("Change cannot be negative", builder.Build().GetError());
