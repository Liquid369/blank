...
UniValue initmasternode(const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.empty() || request.params.size() > 2)) {
        throw std::runtime_error(
                ...
                HelpExampleRpc("initmasternode", "\"9247iC59poZmqBYt9iDh9wDam6v9S1rW5XekjLGyPnDhrDkP4AK\" \"187.24.32.124:56740\""));
    }
...
UniValue listmasternodes(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() > 1))
        throw std::runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode DogeCash address\n"
...
UniValue getmasternodestatus (const JSONRPCRequest& request)
{
    if (request.fHelp || (request.params.size() != 0))
        throw std::runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) DogeCash address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
UniValue getmasternodewinners (const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) DogeCash MN Address\n"
