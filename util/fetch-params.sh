...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/RBXParams"
    else
        PARAMS_DIR="$HOME/.rbx-params"
    fi
fi
...
Failed to fetch the Rubus zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
Rubus - fetch-params.sh

This script will fetch the Rubus zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Rubus zkSNARK parameters. Note that it is
