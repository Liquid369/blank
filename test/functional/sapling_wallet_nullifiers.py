...
class WalletNullifiersTest (PivxTestFramework):
    ...
    def run_test (self):
        ...
        # add shielded addr to node 0
        myzaddr0 = self.nodes[0].getnewshieldedaddress()

        # send node 0 taddr to shielded addr to get out of coinbase
        ...
        # add shielded addr to node 2
        myzaddr = self.nodes[2].getnewshieldedaddress()

        # import node 2 shielded addr into node 1
        ...
        # send node 0  shielded addr to note 2 zaddr
        recipients = []
        ...
        assert_equal(self.nodes[2].getshieldedbalance(myzaddr), zsendmanynotevalue)
        assert_equal(self.nodes[1].getshieldedbalance(myzaddr), zsendmanynotevalue)

        # add shielded addr to node 3
        myzaddr3 = self.nodes[3].getnewshieldedaddress()

        # send node 2 shielded addr to note 3 shielded addr
        recipients = []
        ...
        # send node 2 shielded addr on node 1 to taddr
        ...
        assert_equal(myzaddr in self.nodes[3].listshieldedaddresses(), False)
        assert_equal(myzaddr in self.nodes[3].listshieldedaddresses(True), True)
        ...
        # change are equal.
        node2Received = dict([r['txid'], r] for r in self.nodes[2].listreceivedbyshieldedaddress(myzaddr))
        node3Received = dict([r['txid'], r] for r in self.nodes[3].listreceivedbyshieldedaddress(myzaddr))
        ...
        assert_equal(self.nodes[3].getshieldedbalance(myzaddr), Decimal('0.0'))
        assert_equal(self.nodes[3].getshieldedbalance(myzaddr, 1, True), zaddrremaining2)
