...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        <source>%n active connection(s) to FLITS network</source>
        <translation>
            <numerusform>%n active connection to FLITS network</numerusform>
            <numerusform>%n active connections to FLITS network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `dogecash_en.ts` remains unchanged.
