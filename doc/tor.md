...
your flsd's P2P listen port (51472 by default).
...
In a typical situation, where you're only reachable via Tor, this should suffice:
	...
	./flsd -proxy=127.0.0.1:9050 -externalip=flszj6l4cvo2fxy.onion -listen
...
for normal IPv4/IPv6 communication, use:
	...
	./flsd -onion=127.0.0.1:9050 -externalip=flszj6l4cvo2fxy.onion -discover
...
in `torrc` config file. For cookie authentication the user running flsd must have read
access to the `CookieAuthFile` specified in Tor configuration. In some cases this is
preconfigured and the creation of a hidden service is automatic. If permission problems
are seen with `-debug=tor` they can be resolved by adding both the user running Tor and
the user running flsd to the same group and setting permissions appropriately. On
Debian-based systems the user running flsd can be added to the debian-tor group,
