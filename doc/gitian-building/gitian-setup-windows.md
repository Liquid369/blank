...
The build script we'll be using is contained in the Rubus github repository ([contrib/gitian-build.py](https://github.com/flitsnode/Rubus-core/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the Rubus repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/flitsnode/Rubus-core/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, rbx-detached-sigs, and rbx GitHub repos.
