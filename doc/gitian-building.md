...
*Setup instructions for a gitian build of Rubus Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the RBX
...
to the Rubus GitHub release page.
...
- [Building Rubus Core](#building-rbx-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/rbx-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/rbx-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/rbx-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/rbx-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/rbx-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/rbx-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/rbx-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@rubus.io) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/rbx-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/rbx-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/rbx-osx-*-build.assert
```
