...
- [Development guidelines](#development-guidelines)
    - [General Deviant Core](#general-deviant-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Deviant Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Deviant Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/deviant/deviant/blob/master/contrib/valgrind.supp))
...
However, in open source software such as Deviant Core, where everyone uses
...
pay attention to for reviewers of Deviant Core code.

General Deviant Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Deviant Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Deviant Core so that they can be integrated
...
file descriptors it will cause problems with DEV's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:DEV-Project/DEV.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `deviant-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `deviant-cli` and the GUI debug console use this table to determine how to
