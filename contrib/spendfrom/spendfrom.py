#!/usr/bin/env python
#
# Use the raw transactions API to spend FLSs received on particular addresses,
...
def determine_db_dir():
    """Return the default location of the fls data directory"""
    if platform.system() == "Darwin":
        return os.path.expanduser("~/Library/Application Support/FLS/")
    elif platform.system() == "Windows":
        return os.path.join(os.environ['APPDATA'], "FLS")
    return os.path.expanduser("~/.fls")

def read_bitcoin_config(dbdir):
    """Read the fls.conf file from dbdir, returns dictionary of settings"""
    ...
    config_parser.readfp(FakeSecHead(open(os.path.join(dbdir, "fls.conf"))))
    return dict(config_parser.items("all"))

def connect_JSON(config):
    """Connect to a fls JSON-RPC server"""
...
def list_available(flsd):
    ...
    for output in unspent:
        ...
        # This code only deals with ordinary pay-to-fls-address
...
def main():
    ...
    parser.add_option("--from", dest="fromaddresses", default=None,
                      help="addresses to get FLSs from")
    parser.add_option("--to", dest="to", default=None,
                      help="address to get send FLSs to")
    parser.add_option("--amount", dest="amount", default=None,
                      help="amount to send")
    parser.add_option("--fee", dest="fee", default="0.0",
                      help="fee to include")
    parser.add_option("--datadir", dest="datadir", default=determine_db_dir(),
                      help="location of fls.conf file with RPC username/password (default: %default)")
